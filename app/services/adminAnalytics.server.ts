 ...
 *
 * CACHING:
 * - All analytics functions are cached using Redis
 * - TTLs vary based on data volatility (5min to 30min)
 * - Cache keys follow pattern: admin:analytics:<function>:<params>
 *
 * =============================================================================
 */
...
const CACHE_KEYS = {
  recentSignups: (limit: number) => `admin:analytics:recentSignups:${limit}`,
  creditDistribution: "admin:analytics:creditDistribution",
  zeroCreditUsers: (limit: number) => `admin:analytics:zeroCreditUsers:${limit}`,
  topCreditHolders: (limit: number) => `admin:analytics:topCreditHolders:${limit}`,
  userActivityBreakdown: "admin:analytics:userActivityBreakdown",
  signupTrends: (days: number) => `admin:analytics:signupTrends:${days}`,
  creditFlowSummary: (period: string) => `admin:analytics:creditFlow:${period}`,
  creditTrends: (days: number) => `admin:analytics:creditTrends:${days}`,
  recentCreditTransactions: (limit: number) => `admin:analytics:recentTransactions:${limit}`,
  generationStats: (period: string) => `admin:analytics:generationStats:${period}`,
  modelUsageBreakdown: (period: string) => `admin:analytics:modelUsage:${period}`,
  generationTrends: (days: number) => `admin:analytics:generationTrends:${days}`,
  modelSuccessRates: (period: string) => `admin:analytics:modelSuccessRates:${period}`,
  adminDashboardStats: "admin:analytics:dashboardStats",
  modelPopularityTrends: (days: number) => `admin:analytics:modelPopularity:${days}`,
  modelRankings: (period: string) => `admin:analytics:modelRankings:${period}`,
  modelAdoptionTrends: (days: number) => `admin:analytics:modelAdoption:${days}`,
  topUsersByFollowers: (limit: number) => `admin:analytics:topByFollowers:${limit}`,
  topCreatorsByEngagement: (limit: number) => `admin:analytics:topByEngagement:${limit}`,
  platformSocialStats: "admin:analytics:socialStats",
  mostLikedContent: (limit: number) => `admin:analytics:mostLiked:${limit}`,
  followTrends: (days: number) => `admin:analytics:followTrends:${days}`,
} as const;

// Cache TTLs in seconds
const CACHE_TTL = {
  SHORT: 300,      // 5 minutes - for frequently changing data (today's stats, recent transactions)
  MEDIUM: 900,     // 15 minutes - for moderately fresh data (dashboard stats, activity)
  LONG: 1800,      // 30 minutes - for stable data (trends, rankings, distributions)
} as const;
...
export async function getRecentSignups(limit: number = 20): Promise<RecentUser[]> {
  return getCachedData(
    CACHE_KEYS.recentSignups(limit),
    async () => {
...
export async function getZeroCreditUsers(limit: number = 50) {
  return getCachedData(
    CACHE_KEYS.zeroCreditUsers(limit),
    async () => {
      return prisma.user.findMany({
        where: { credits: 0 },
        orderBy: { createdAt: "desc" },
...
export async function getTopCreditHolders(limit: number = 20) {
  return getCachedData(
    CACHE_KEYS.topCreditHolders(limit),
    async () => {
      return prisma.user.findMany({
...
export async function getUserActivityBreakdown(): Promise<UserActivityBreakdown> {
  return getCachedData(
    CACHE_KEYS.userActivityBreakdown,
    async () => {
      const now = new Date();
 ...
 * @cached TTL: 5 minutes for "today", 15 minutes for others
 */
export async function getCreditFlowSummary(
  period: "today" | "week" | "month" | "all" = "all"
): Promise<CreditFlowSummary> {
  // Use shorter TTL for "today" since it changes frequently
  const ttl = period === "today" ? CACHE_TTL.SHORT : CACHE_TTL.MEDIUM;

  return getCachedData(
    CACHE_KEYS.creditFlowSummary(period),
    async () => {
      const now = new Date();
...
export async function getRecentCreditTransactions(limit: number = 50) {
  return getCachedData(
    CACHE_KEYS.recentCreditTransactions(limit),
    async () => {
...
export async function getGenerationStats(
  ...
  return getCachedData(
    CACHE_KEYS.generationStats(period),
    async () => {
      const now = new Date();
      let startDate: Date | undefined;
...
export async function getModelUsageBreakdown(
  ...
  return getCachedData(
    CACHE_KEYS.modelUsageBreakdown(period),
    async () => {
      const now = new Date();
      let startDate: Date | undefined;
...
export async function getGenerationTrends(days: number = 30): Promise<GenerationTrend[]> {
  return getCachedData(
    CACHE_KEYS.generationTrends(days),
    async () => {
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - days);
      startDate.setHours(0, 0, 0, 0);
...
export async function getModelSuccessRates(
  ...
  return getCachedData(
    CACHE_KEYS.modelSuccessRates(period),
    async () => {
      const now = new Date();
      let startDate: Date | undefined;
...
export async function getAdminDashboardStats(): Promise<AdminDashboardStats> {
  return getCachedData(
    CACHE_KEYS.adminDashboardStats,
    async () => {
...
export async function getModelPopularityTrends(days: number = 30): Promise<ModelPopularityTrend[]> {
  return getCachedData(
    CACHE_KEYS.modelPopularityTrends(days),
    async () => {
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - days);
      startDate.setHours(0, 0, 0, 0);
...
export async function getModelRankings(
  ...
  return getCachedData(
    CACHE_KEYS.modelRankings(period),
    async () => {
      const now = new Date();
...
export async function getModelAdoptionTrends(days: number = 30) {
  return getCachedData(
    CACHE_KEYS.modelAdoptionTrends(days),
    async () => {
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - days);
      startDate.setHours(0, 0, 0, 0);
...
export async function getTopUsersByFollowers(limit: number = 20): Promise<UserSocialMetrics[]> {
  return getCachedData(
    CACHE_KEYS.topUsersByFollowers(limit),
    async () => {
      const users = await prisma.user.findMany({
        orderBy: {
          followedBy: {
...
export async function getTopCreatorsByEngagement(limit: number = 20): Promise<UserSocialMetrics[]> {
  return getCachedData(
    CACHE_KEYS.topCreatorsByEngagement(limit),
    async () => {
...
export async function getPlatformSocialStats(): Promise<PlatformSocialStats> {
  return getCachedData(
    CACHE_KEYS.platformSocialStats,
    async () => {
      const [
        totalFollows,
        totalUsers,
...
export async function getMostLikedContent(limit: number = 10) {
  return getCachedData(
    CACHE_KEYS.mostLikedContent(limit),
    async () => {
