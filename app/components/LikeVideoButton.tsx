...
interface LikeVideoButtonProps {
  videoData: {
    id?: string;
...
export const LikeVideoButton = ({ videoData }: LikeVideoButtonProps) => {
  ...
  const [isLiked, setIsLiked] = React.useState(
    videoData.likes?.some((like) => like.userId === userData?.id) ?? false
  );
  ...
  const handleClick = async () => {
    if (disableButton || !videoData.id) return;
    ...
    try {
      fetcher.submit(
        { videoId: videoData.id },
        {
          ...
          action: `/api/videos/${videoData.id}/like`,
        }
      );
    } catch (error) {
      // Revert on error
